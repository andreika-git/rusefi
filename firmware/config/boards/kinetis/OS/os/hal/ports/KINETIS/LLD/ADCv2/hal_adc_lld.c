...
*/
	adcp->current_channel = 14;

#ifdef KE1xF
    adcp->adc12ChannelCfg.channelNumber = adcp->current_channel;
...
void adc_lld_start_conversion(ADCDriver *adcp) {
  ...
  int chn;
  adcp->channel_mask = 0;
  for (chn = 0; chn < 10; chn++) {
    if ((grpp->smpr2 >> (chn * 3)) & 7) {	// ADC_SMPR2_SMP_ANx(n)
    	adcp->channel_mask |= (1 << chn);
    }
  }
  for (chn = 10; chn < 16; chn++) {
    if ((grpp->smpr1 >> ((chn - 10) * 3)) & 7) { // ADC_SMPR1_SMP_ANx(n)
    	adcp->channel_mask |= (1 << chn);
    }
  }
  // todo: also check sqr1,sqr2,sqr3
  // see: AdcDevice::enableChannel()
