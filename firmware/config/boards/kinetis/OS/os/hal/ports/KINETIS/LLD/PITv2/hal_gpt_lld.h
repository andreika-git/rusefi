...
#define gpt_lld_change_interval(gptp, interval)                               \
  (PIT->CHANNEL[(gptp)->channelIndex].TVAL = (uint32_t)( \
                            ( (gptp)->clock / (gptp)->config->frequency ) * \
                            ( interval ) ))
#else
#define gpt_lld_change_interval(gptp, interval)                               \
  ((gptp)->channel->TVAL = (uint32_t)( \
                            ( (gptp)->clock / (gptp)->config->frequency ) * \
                            ( interval ) ))
#endif
...
#define gpt_lld_get_interval(gptp)                                            \
  ((uint32_t)( ( (uint64_t)PIT->CHANNEL[((gptp)->channelIndex].TVAL * (gptp)->config->frequency ) / \
               ( (uint32_t)(gptp)->clock ) ))
#else
#define gpt_lld_get_interval(gptp)                                            \
  ((uint32_t)( ( (uint64_t)(gptp)->channel->TVAL * (gptp)->config->frequency ) / \
               ( (uint32_t)(gptp)->clock ) ))
#endif
 ...
 */
#ifdef KE1xF
#define gpt_lld_get_counter(gptp) ((gptcnt_t)PIT->CHANNEL[(gptp)->channelIndex].CVAL)
#else
/* [andreika]: it seems that something is wrong here in the original code... */
